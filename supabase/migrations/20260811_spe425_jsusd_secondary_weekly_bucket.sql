...
--
-- Idempotent: after it runs no in-scope student has sessions_per_week > 1, so a
-- second run matches nothing. Both writes live in one DO block so the fix is
-- all-or-nothing however it is applied — a student left at 1 × weekly total
-- while still holding 19 templates would read as ~19× their real caseload.
...
DECLARE
  ...
  v_templates integer;
BEGIN
  -- Pre-flight. This sweep was rehearsed against a caseload in which every
  -- in-scope template is still an untouched, unscheduled placeholder: nothing
  -- scheduled, completed, grouped, assigned, and no instance, attendance, or
  -- curriculum history hanging off it. If that stopped being true — a provider
  -- placed one of the phantom pull-outs on their calendar, say — then step 1
  -- would rewrite the student to a 570-minute weekly requirement while leaving
  -- 30-minute blocks on the schedule, and step 2's guards would (correctly)
  -- refuse to prune them. That half-fixed state is worse than no fix and is a
  -- human decision, so stop and say so rather than writing it silently.
  ...
  SET sessions_per_week = 1,
      ...
      updated_at = NOW()
  FROM profiles p
  WHERE p.id = st.provider_id
    AND st.school_id IN ('061899002299', '061899002302')
    AND p.role = 'resource'
    AND st.sessions_per_week > 1
  ...
  WITH scoped AS (
    SELECT st.id AS student_id, st.sessions_per_week AS keep_n
    FROM students st
    JOIN profiles p ON p.id = st.provider_id
    WHERE st.school_id IN ('061899002299', '061899002302')
      AND p.role = 'resource'
      AND st.sessions_per_week IS NOT NULL
  ),
  ranked AS (
    SELECT ss.id, s.keep_n,
           ROW_NUMBER() OVER (
    ...
    FROM schedule_sessions ss
    JOIN scoped s ON s.student_id = ss.student_id
    WHERE ss.session_date IS NULL   -- templates only; dated instances are history
      AND ss.deleted_at IS NULL
  )
