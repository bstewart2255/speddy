-- Update atomic function for upserting students to also update teacher_name
-- This fixes the issue where teacher_id was updated but teacher_name wasn't
        ...
        WHEN 'update' THEN
          ...
          SET
            ...
            teacher_id = CASE
              ...
              ELSE teacher_id
            END,
            -- Also update teacher_name (deprecated field but still used for display)
            teacher_name = CASE
...
COMMENT ON FUNCTION public.upsert_students_atomic(uuid, jsonb) IS
  'Atomically processes an array of student upsert operations (insert, update, or skip).
   ...
   Now also updates teacher_name (deprecated field) alongside teacher_id for display compatibility.
   Returns summary counts and detailed results for each student.';

-- One-time backfill: populate teacher_name for students that have teacher_id but no teacher_name
-- This fixes students that were updated before this fix was applied
