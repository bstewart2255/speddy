-- Fix Bug #3: Use provider's role for service_type instead of hardcoding 'speech_therapy'
-- See GitHub issue #459 for full context on session creation pipeline bugs
...
COMMENT ON FUNCTION public.upsert_students_atomic(uuid, jsonb) IS
  'Atomically processes an array of student upsert operations (insert, update, or skip).
   ...
   Uses provider role for service_type instead of hardcoded value.
