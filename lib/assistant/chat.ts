...
const SPEDDY_GUIDE = `- Layout: the nav bar has Dashboard, Students, Schedule (dropdown: Main Schedule, Bell Schedules, Special Activities), Meetings, Plan, and Chat — plus Referrals for resource specialists. Settings and Sign out are under the avatar circle at the top right, and the "Ask AI" button beside it opens this assistant.
...
- Dashboard: "Today's Schedule" with the day's sessions, a setup checklist card beside it (Add your students, Set your work schedule, Enter bell schedules, Enter special activities, Schedule your sessions — each ticks itself off as the data arrives, and the card can be dismissed), the "This Week's Attendance" widget, and a personal "To-Do List" widget ("+ Add").
- Middle and high school (secondary) sites work differently, by role. Resource specialists keep Schedule and Bell Schedules, plan service as weekly minutes rather than sessions, and their Schedule page is a periods-by-days week view instead of the drag-and-drop grid. Speech, OT, counseling and school psychologists keep the full scheduling grid, minus Special Activities. Other roles lose Schedule, Bell Schedules, Special Activities and Plan entirely. The "This Week's Attendance" widget and a student's Attendance tab are elementary-only.
- Add a student: Students page → "+ Add Student" → enter Student Initials, Grade Level, Teacher, Sessions/Week and Min/Session → "Add & add another" saves and keeps the form open for the next student; "Done" closes it. At a secondary site the form asks for Teachers (more than one allowed) and Minutes/Week instead. The "As the IEP states it:" row converts the IEP's own wording — minutes per day, week, month or year — into the numbers the form wants.
...
- Accommodations tab: list a student's IEP accommodations, added by hand, or lifted from an uploaded IEP with "Import from IEP PDF" (the provider reviews and confirms each one before it is saved).
...
- Secondary resource specialists record where they see each student instead of dragging sessions: Schedule → "Add Service Time" → pick the Student, the Setting ("My room" or "Push-in"), the Class, the Period and the days. The same list appears as "Where I see this student" on the student's Current Information tab. Entering the school's period grid under Bell Schedules first gives real period names to choose from. Recorded push-ins are visible to colleagues: anyone who tries to schedule a pull-out session over that time is warned that a provider is already with the student in that class.
...
- Special Activities (under Schedule): teacher-specific activities like music or library — "+ Add Activity" or "Import CSV". Any activity name can be typed; it is not limited to a fixed list. Together with bell schedules, these keep the auto-scheduler from placing sessions at bad times.
