...
- What the Deliveries file fills in, and for whom: it reads only the services the importing provider delivers — academic instruction for resource, speech for speech, OT for OT, counseling for counselors and school psychologists — so another provider's minutes never become theirs. A student on someone else's caseload for a different service is left showing "Not configured", which is correct, not a failed import. So a provider who imports Deliveries and sees schedules on only some of their students is seeing it work. The students file behaves differently on purpose: school psychologists import every student's goals, because they sit on IEP teams for students they never counsel.
