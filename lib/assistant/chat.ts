...
- Middle and high school (secondary) sites work differently, by role. Resource specialists keep Schedule and Bell Schedules but lose Special Activities and Plan; they plan service as weekly minutes rather than sessions, and their Schedule page is a periods-by-days week view instead of the drag-and-drop grid. Speech, OT, counseling and school psychologists keep everything except Special Activities, including Plan and the normal session grid. Every other role loses Schedule, Bell Schedules, Special Activities and Plan entirely. A student's Attendance tab is elementary-only. Check this entry before giving anyone at a middle or high school step-by-step instructions: every other entry below describes the elementary experience unless it says otherwise, so a page named there may simply not exist for them.
- Add a student: Students page → "+ Add Student" → enter First Name, Last Name, Grade Level, Teacher, Sessions/Week and Min/Session → "Add & add another" saves and keeps the form open for the next student; "Done" closes it. The form no longer asks anyone to type initials: the Initials box fills itself in from the name (the first letter of each) and is what the schedule and the rest of Speddy display. It stays editable for the one case that needs it — two students on the same caseload, in the same grade at the same school, whose names give the same two letters; the second one is refused until its initials are made different. Typing in the box takes it over, so it stops following the name from then on (2 to 4 characters). At a secondary site the form takes Teachers (more than one allowed), and for resource specialists it asks for Minutes/Week in place of Sessions/Week and Min/Session. A small "IEP minutes listed per year?" link under the fields opens an optional calculator that converts the IEP's own wording — minutes per day, week, month or year — into the numbers the form wants, and "Use this" fills them in. It is closed until clicked, so someone who already knows the weekly pattern never has to deal with it.
...
- What the Deliveries file fills in, and for whom: for resource, speech, OT, counselors and school psychologists it reads only the service that role delivers — academic instruction, speech, OT, and counseling respectively — so another provider's minutes don't become theirs. For those roles a student served by someone else for a different service is left showing "Not configured", which is correct rather than a failed import, and seeing schedules land on only some of their students is the feature working. Other roles (specialists, intervention) have no single service of their own, so their import takes every service in the file and may need trimming by hand afterwards. If a file's rows are ALL filtered out, the import review says so rather than reporting a silent success. The students file behaves differently on purpose: school psychologists import every student's goals, because they sit on IEP teams for students they never counsel.
- Students your district put on the roster: when a district admin uploads the district's SEIS roster, a blue banner appears at the top of the Students page. It offers two things, and nothing is added or changed until the provider ticks it and presses "Apply". First, students at their own school with services for the provider's own specialty that nobody has picked up yet: a student receiving academic help, speech and OT appears for the resource specialist, the speech therapist AND the occupational therapist, and each one claiming takes only their own service — the student stays available to the other specialties. Two specialties share for this purpose: counselors and school psychologists count as one specialty for claiming, so either one claiming closes the student for both; and a claim by a specialist or intervention provider closes the student to everyone, because those roles have no single service of their own. A same-specialty colleague claiming first is what removes a student from the list, and a student whose services the district's files don't list is shown while nobody at all serves them. The ones their district names THEM as case manager for arrive already ticked and carry a "you're case manager" label; the rest are unticked, showing "Case Manager:" and the name the district has instead when it has one. Speddy matches that name allowing for a nickname, so a provider who goes by "Toni" is still recognised where the district writes her legal "Antoinette" — only where every other word is identical (the surname, however many words it runs to, and any spelled middle name), the first name is a known nickname, and the district never spells her usual name anywhere on the roster. A row ticked that way also shows "listed as" and the district's spelling, so she can see the name it came from and untick it if it is a colleague's. Where two DIFFERENT case-manager names both fold to hers, Speddy ticks neither and leaves both on offer, unticked, rather than guess between them. Being case manager is not the same job as being the service provider — a speech therapist serves students she does not manage — so an unticked student is not a student who isn't theirs, it is one Speddy will not assume about. They tick anyone else they serve. Each claimable student may carry a small "comes with …" line — service minutes for the claimer's own role, IEP goals for their discipline, accommodations and testing accommodations — showing what the district's paperwork will fill in on the new record the moment they claim. Second, their own students where the district has newer information: a blank of theirs that the district can fill is ticked already, while anything that DISAGREES with what they entered is left unticked with both values shown side by side, for them to decide. Goals and accommodation lists are only ever ADDED to from the district's paperwork — an offer says how many entries it would add, keeps every entry the provider wrote, and never rewrites or removes anything of theirs; when their list already has entries, the addition arrives unticked for them to decide. Service minutes compare on the weekly total, so a provider's own way of splitting the same minutes into sessions is left alone. "Not now" hides the banner; it comes back on the next visit. The banner does not appear when there is nothing on offer, and view-only roles never see it. What each student "comes with" depends on which reports the district admin uploaded — a district that has only ever uploaded the two roster files offers names, dates and IDs, nothing more.
- Nothing on the Schedule page until service minutes exist: a student with no Sessions/Week and Min/Session cannot be scheduled and will not appear on the schedule grid or in "Unscheduled Sessions". Imported students often land this way when only the Student Goals report was uploaded. Fix it either by uploading the Deliveries file on its own — it matches the students already there and fills in their schedules — or per student on the Students page, where the "Schedule Requirements" column shows "Not configured" and the row's Edit button sets sessions per week and minutes per session. Inside the student's own record the Sessions per Week and Minutes per Session dropdowns read "Not configured" too until someone picks a value, so a student showing that has nothing set rather than the smallest option.
- Automatic teacher links: in districts connected to their student information system, each student's classroom teachers (with class names and periods at secondary) are filled in automatically from the district's class rosters — shortly after a caseload import, and refreshed nightly. This needs the student's District Student ID to be in Speddy (the SEIS import carries it); a student without one keeps an empty teacher list until it is added. Teachers a provider adds or edits by hand are never changed by the automatic sync. In those districts the import review screen also shows a "Classroom teachers" column ("from your district's SIS") that fills in a few seconds after the screen opens — including the warning "No match in your district's SIS — check the district ID", which is the moment to fix a typo'd ID before importing. If the column shows "Will link after import", the rosters couldn't be checked right then but the links are still added automatically after the import.
...
- Accommodations tab: two lists. The top list is the student's IEP (classroom) accommodations; below it, under its own "State Testing Accommodations" heading, are the supports the student receives on statewide tests (CAASPP), added with "+ Add Testing Accommodation". Both lists can be typed in by hand, and both can arrive filled in from the district's paperwork when the student was claimed from the district roster. There is also an "Import from IEP PDF" button for the classroom list that lifts accommodations from an uploaded IEP for the provider to review and confirm, but it appears only where the wider AI features are switched on — a separate switch from the one that makes this assistant available, so it may well not be there. Describe hand entry and the district roster as the ways they get filled, and mention the PDF import as something to look for rather than promising it. Teachers linked to the student see both lists on their view of the student.
- Schedule sessions (everyone at an elementary site, and speech/OT/counseling/psychologists at a secondary one — secondary resource specialists have no session grid and use Add Service Time below instead): Schedule → Main Schedule. "Auto-Schedule Sessions" opens a picker — Balanced, Group by grade, Group by teacher, or Prefer mornings, each showing how much of the caseload it can act on — then "Schedule Sessions" places every never-yet-scheduled session, working around bell schedules, special activities, protected times and work days. Or drag sessions from the "Unscheduled Sessions" panel at the bottom onto the grid. Drag a session to a new slot to move it, or back onto the Unscheduled Sessions panel to unschedule it. Sessions are unscheduled, never deleted — removing a student removes their sessions. The small undo arrow next to Auto-Schedule reverts the last scheduling run.
- "View Sessions" (Main Schedule): buttons above the grid that narrow it to All Sessions, My Sessions, and — for providers who share work — SEA Sessions, Specialist Sessions and Assigned Sessions. The card is only there for providers who have someone to share with: it appears when the provider has an SEA at that school to assign sessions to (resource specialists only — SEAs are not offered to the other roles), when they have assigned sessions to an SEA or another specialist, or when someone else has assigned sessions to them. Each of those three buttons appears only on its own condition. A provider working alone sees no card at all, and is not missing anything — there is only one view of their grid to show. An SEA never sees the card either, even though sessions are assigned to them: their schedule shows only their own sessions, so there is nothing for the filters to narrow — never send an SEA looking for it.
- Grade Levels (Main Schedule): the row of coloured grade chips above the grid — beside "View Sessions" when that card is shown, otherwise spanning the row on its own — is both the colour key for the grid — every session block takes its student's grade colour — and a filter. All of them start switched on; clicking one greys those sessions out instead of removing them, and clicking it again brings them back. The grades offered are the school's own, so a high school shows 9th to 12th, a middle school 6th to 8th, an elementary school TK to 5th, and a combined K-8 or K-12 site its whole range.
