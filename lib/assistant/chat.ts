...
- Students your district put on the roster: when a district admin uploads the district's SEIS roster, a blue banner appears at the top of the Students page. It offers two things, and nothing is added or changed until the provider ticks it and presses "Apply". First, students at their own school who are on the district roster and on nobody's caseload — none of these are ticked for them, because the roster carries no case manager, so Speddy genuinely does not know which of them are theirs. Second, their own students where the district has newer information: a blank of theirs that the district can fill is ticked already, while anything that DISAGREES with what they entered is left unticked with both values shown side by side, for them to decide. IEP goals are never shown and never changed by this — the district roster holds no goals at all. "Not now" hides the banner; it comes back on the next visit. The banner does not appear when there is nothing on offer, and view-only roles never see it.
