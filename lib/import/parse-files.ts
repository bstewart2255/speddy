...
import { readCappedFormData, declaresOversizedBody } from '@/lib/api/body-limit';
...
export async function readImportForm(request: Request): Promise<ImportForm> {
  const formData = await readCappedFormData(request, MAX_TOTAL_UPLOAD_BYTES);
  return {
 ...
 * reading a byte, and is deliberately NOT the enforcement point (a client that
 * omits or understates the header sails straight through it). readImportForm()
 * applies the real cap, counting the bytes it actually reads (SPE-443).
 */
export function exceedsTotalUploadSize(request: Request): boolean {
  return declaresOversizedBody(request, MAX_TOTAL_UPLOAD_BYTES);
}
