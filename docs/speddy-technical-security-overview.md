...
- **Prompt de-identification is implemented.** Speddy does not place any student identifier from its own records into a prompt: lesson prompts reference students only by positional labels ("Student 1", "Student 2") that are mapped back to initials inside Speddy before anything is displayed or printed, and exit-ticket and progress-check prompts carry only a grade level and IEP goal text. No student initials, names, or record identifiers are drawn from the database into a prompt
- **Three content paths carry text or imagery authored outside Speddy's control, and are disclosed rather than claimed clean.** Speddy does not currently redact any of them:
