...
- **OpenAI / Anthropic (other AI features)** — *only when `AI_FEATURES_ENABLED` is on* (currently hard-gated off, SPE-162 — gated routes 404 with **zero** provider calls): student **initials + IEP goal text** (lessons / exit-tickets / progress-checks), worksheet **images** (vision grading), uploaded-document text, and **uploaded IEP PDFs** (accommodations import, SPE-489 — the full PDF content transits Anthropic for one extraction call; Speddy never writes the file to disk or storage, and only the provider-approved accommodations list is saved). De-identification tracked in SPE-61.
