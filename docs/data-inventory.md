...
- **Anthropic (accommodations import)** — *only when `AI_FEATURES_ENABLED` is on* (currently hard-gated off, SPE-162 — the gated route 404s with **zero** provider calls): **uploaded IEP PDFs** (SPE-489 — the full PDF content transits Anthropic for one extraction call; Speddy never writes the file to disk or storage, and only the provider-approved accommodations list is saved). The rest of the gated AI suite (lesson / exit-ticket / progress-check generation, worksheet vision) and its OpenAI provider were **removed from the codebase 2026-08 (SPE-497)**; the retained rows in the tables above are historical data pending the stage-2 table cleanup.
