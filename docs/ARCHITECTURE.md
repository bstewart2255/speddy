...
| Grade Levels filter | **Not secondary-gated — school-derived** (`getSchoolGradeRange`, SPE-587): a high school offers 9th–12th, a middle school 6th–8th, a K-8 combined site TK–8, and everything else falls back to TK–5. All of them start selected. Deriving from the span rather than branching on `isSecondary` is what gets a combined site its upper grades, since K-8/K-12 classify as elementary above. A student outside the school's range (a 5th grader recorded at a high school, an unreadable imported grade — SPE-467) is never dimmed: the filter can't offer a toggle to restore them | `lib/school-helpers.ts` (`getSchoolGradeRange`); `schedule/components/schedule-controls.tsx`; `schedule/hooks/use-schedule-state.tsx`; `schedule/components/schedule-grid.tsx`; `lib/scheduling/constants.ts` (the TK–12 palette) |
