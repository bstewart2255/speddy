...
| 9 | Derek Holloway-Sim | `resource` | Juniper | `rsp.juniper` | Full-time RSP #3 (24 students); rounds out one-per-elementary coverage. **Also the SDC dual-role persona (SPE-355/478):** his account is linked to his own classroom-teacher entry at Juniper (`teachers.account_id`), which is the marker that unlocks the SDC scheduling affordances (mainstreaming blocks) |
